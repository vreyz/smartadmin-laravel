...
`Vreyz\Admin\Widgets\Box`用来生成box组件：
...
`Vreyz\Admin\Widgets\Collapse`类用来生成折叠插件：
```php
...
`Vreyz\Admin\Widgets\Form`类用来快速构建表单：
...
`Vreyz\Admin\Widgets\InfoBox`类用来生成信息展示块：
...
`Vreyz\Admin\Widgets\Tab`类用来生成选项卡组件：
...
`Vreyz\Admin\Widgets\Table`类用来生成表格：
