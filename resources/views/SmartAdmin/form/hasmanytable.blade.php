        ...
        <div id="has-many-{{$column}}" style="margin-top: 15px;">
                ...
                @foreach($forms as $pk => $form)
                    <tr class="has-many-{{$column}}-form fields-group">
                        ...
                        @foreach($form->fields() as $field)
                            ...
                            @if (is_a($field, \Vreyz\Admin\Form\Field\Hidden::class))
                        ...
                        @if($options['allowDelete'])
                            <td class="form-group">
                                <div>
                                    <div class="remove btn btn-warning btn-sm float-right"><i class="fal fa-trash">&nbsp;</i>{{ trans('admin.remove') }}</div>
                                </div>
                            </td>
                        @endif
                    </tr>
            ...
            @if($options['allowCreate'])
                <div class="form-group">
                    <div class="{{$viewClass['field']}}">
                        <div class="add btn btn-success btn-sm"><i class="fal fa-save"></i>&nbsp;{{ trans('admin.new') }}</div>
                    </div>
                </div>
            @endif
        </div>
