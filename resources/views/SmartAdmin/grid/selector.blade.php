            ...
            <div class="select-options">
                <ul>
                    @foreach($selector['options'] as $value => $option)
                        ...
                        @endphp
                        <li>
                            <a href="{{ \Vreyz\Admin\Grid\Tools\Selector::url($column, $value, true) }}"
                               class="{{$active ? 'active' : ''}}">{{ $option }}</a>
                            @if(!$active && $selector['type'] == 'many')
                                &nbsp;
                                <a href="{{ \Vreyz\Admin\Grid\Tools\Selector::url($column, $value) }}" class="add"><i
                                            class="fal fa-plus-square-o"></i></a>
                            @else
                                <a style="visibility: hidden;"><i class="fal fa-plus-square-o"></i></a>
                            @endif
                        </li>
                    @endforeach
                    <li>
                        <a href="{{ \Vreyz\Admin\Grid\Tools\Selector::url($column) }}" class="clear"><i
